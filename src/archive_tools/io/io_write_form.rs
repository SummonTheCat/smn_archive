...
pub fn write_form(file_path: &str, form: &dyn FormTrait) -> std::io::Result<()> {
    ...
    if archive_info.form_count == 0 {
        ...
        if found_form {
            ...
            form_length_diff = form_length_new - form_length_old;
...
pub fn write_form_over_existing(file: &mut File, archive: &Archive, form_bytes: &Vec<u8>, form_index: &mut IOStructIndex, form_index_position: u32, form_length_diff: u32) -> std::io::Result<()> {
    file.seek(SeekFrom::Start(0))?;
    ...
    let next_form_start_pos = form_index.indexes[form_index_position as usize + 1].data_start_offset + archive.bytestart_data - form_length_diff;
